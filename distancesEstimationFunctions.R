...
#Returns:
#	dists: an array with the distances between each pair of units of size N*(N-1)/2 x (vars+pvars)
...
# this method is called by getDistances when all variables are deterministic
...
# this method is called by getDistances when confounding variables are deterministic but the treatment is probabilistic
...
# this method is called by getDistances when confounding variables are probabilistic but the treatment is deterministic
...
# this method is called by getDistances when all variables are probabilistic
...
# estimate a 'distance' metric based on the area covered by the CDF
# Args:
# 	myCdf: cdf i.e. (F(x))
# 	x: x
# Returns: 
# 	d: area covered by the CDF
